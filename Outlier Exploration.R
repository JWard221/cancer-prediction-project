...
fit <- lm(avgAnnCount ~. -1, data = scaled.train)
summary(fit)
...
# Now Looking at the model with 8 variables
names8 <- c(names8,'avgAnnCount')
train <- train[, which(names(train) %in% names8)]
n <- dim(train)[1]
k <- dim(train)[2]
tr.names <- names(train)
...
# In this subset, these variables seem important 
#[1] "473"  "782"  "996"  "1490" "1537" "1607" "1638" "1728" "2021" "2043" "2273"
