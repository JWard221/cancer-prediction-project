...
### Final model -
# Our final model is lm1_v5
# Test model for multicollinearity
vif(lm1_v5)

# GVIF Df GVIF^(1/(2*Df))
# poly(medIncome, 2, raw = T)          7.261600  2        1.641565
# PercentMarried                       8.449993  1        2.906887
# PctHS25_Over                         3.040349  1        1.743660
# poly(PctBachDeg25_Over, 3, raw = T)  7.889593  3        1.410942
# PctEmployed16_Over                   5.113763  1        2.261363
# PctUnemployed16_Over                 2.445216  1        1.563718
# PctPrivateCoverage                   7.087500  1        2.662236
# PctPublicCoverage                   11.004361  1        3.317282
# PctPublicCoverageAlone              13.680054  1        3.698656
# PctMarriedHouseholds                 7.506780  1        2.739850
# BirthRate                            1.100788  1        1.049185

# We see multicollinearity present, so we drop PctPublicCoverage
# Dropping this makes PctEmployed16_Over insignificant, so we drop that too.

lm1_v5 = lm(TARGET_deathRate ~ poly(medIncome,2,raw=T)+PercentMarried+PctHS25_Over+poly(PctBachDeg25_Over,3,raw=T)+PctUnemployed16_Over+
              PctPrivateCoverage+PctPublicCoverageAlone+PctMarriedHouseholds+BirthRate , data = train_1)
summary(lm1_v5)
# Call:
#   lm(formula = TARGET_deathRate ~ poly(medIncome, 2, raw = T) + 
#        PercentMarried + PctHS25_Over + poly(PctBachDeg25_Over, 3, 
#                                             raw = T) + PctUnemployed16_Over + PctPrivateCoverage + PctPublicCoverageAlone + 
#        PctMarriedHouseholds + BirthRate, data = train_1)
# 
# Residuals:
#   Min     1Q Median     3Q    Max 
# -86.74 -11.99   0.57  12.70 143.55 
# 
# Coefficients:
#   Estimate Std. Error t value Pr(>|t|)    
# (Intercept)                           2.228e+02  1.349e+01  16.520  < 2e-16 ***
#   poly(medIncome, 2, raw = T)1         -6.505e-04  2.536e-04  -2.566 0.010366 *  
#   poly(medIncome, 2, raw = T)2          6.479e-09  2.013e-09   3.218 0.001307 ** 
#   PercentMarried                        3.804e-01  1.536e-01   2.478 0.013301 *  
#   PctHS25_Over                          7.522e-01  1.114e-01   6.751 1.86e-11 ***
#   poly(PctBachDeg25_Over, 3, raw = T)1 -7.028e+00  1.096e+00  -6.412 1.75e-10 ***
#   poly(PctBachDeg25_Over, 3, raw = T)2  3.333e-01  6.335e-02   5.262 1.56e-07 ***
#   poly(PctBachDeg25_Over, 3, raw = T)3 -5.829e-03  1.137e-03  -5.125 3.23e-07 ***
#   PctUnemployed16_Over                  9.596e-01  2.025e-01   4.737 2.30e-06 ***
#   PctPrivateCoverage                    2.465e-01  1.086e-01   2.270 0.023327 *  
#   PctPublicCoverageAlone                6.248e-01  1.821e-01   3.432 0.000611 ***
#   PctMarriedHouseholds                 -1.089e+00  1.619e-01  -6.724 2.24e-11 ***
#   BirthRate                            -1.393e+00  2.507e-01  -5.557 3.07e-08 ***
#   ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 21.66 on 2248 degrees of freedom
# Multiple R-squared:  0.3605,	Adjusted R-squared:  0.3571 
# F-statistic: 105.6 on 12 and 2248 DF,  p-value: < 2.2e-16
...
res1 = resid(lm1_v5)
# get hat values
hat1 = hatvalues(lm1_v5)
# get studentized residuals
stud1 = rstandard(lm1_v5)
# get r student residuals
rstud1 = rstudent(lm1_v5)
# create a dataframe with this info
resid_df = as.data.frame(cbind(res1,hat1,stud1,rstud1))

# create a normal probability plot of residuals
qqnorm(resid_df$rstud1, datax=TRUE, pch=16, cex=1, xlab="percent", ylab="R-student residual", main = "Normal probability plot of residuals")
qqline(resid_df$rstud1)
# slightly heavy-tailed distribution 

# get index of outlier showed in qqplot
which(resid_df$rstud1 > 6)
# [1] 301

# remove observation 301
train_1 = train_1[-301,]
resid_df = resid_df[-301,]

# recreate a normal probability plot of residuals
qqnorm(resid_df$rstud1, datax=TRUE, pch=16, cex=1, xlab="percent", ylab="R-student residual", main = "Normal probability plot of residuals")
qqline(resid_df$rstud1)
# looks much better after moving outlier 

# retrain model with outlier removed
lm1_v5 = lm(TARGET_deathRate ~ poly(medIncome,2,raw=T)+PercentMarried+PctHS25_Over+poly(PctBachDeg25_Over,3,raw=T)+PctUnemployed16_Over+
              PctPrivateCoverage+PctPublicCoverageAlone+PctMarriedHouseholds+BirthRate , data = train_1)
summary(lm1_v5)
par(mfrow = c(1, 2))
# residual-by-fitted-value-plot
yhat = predict(lm1_v5, train_1)
plot(yhat, resid_df$rstud1, pch=16, cex=1, xlab="fitted value", ylab="R-student residual", main = "Residual-by-fitted-value plot")
abline(h=0, lty=1, lwd=3)
# It appears the residuals can be contained in a horizontal band

# # residual-by-regressor plots
par(mfrow = c(2, 2))
plot(train_1$PctUnemployed16_Over, resid_df$rstud1, pch=16, cex=1, xlab="PctUnemployed16_Over", ylab="R-student residual", main = "Residual-by-regressor plot")
abline(h=0, lty=1, lwd=3)
plot(train_1$PctHS25_Over, resid_df$rstud1, pch=16, cex=1, xlab="PctHS25_Over", ylab="R-student residual", main = "Residual-by-regressor plot")
abline(h=0, lty=1, lwd=3)
plot(train_1$PctPublicCoverageAlone, resid_df$rstud1, pch=16, cex=1, xlab="PctPublicCoverageAlone", ylab="R-student residual", main = "Residual-by-regressor plot")
abline(h=0, lty=1, lwd=3)
plot(train_1$PercentMarried, resid_df$rstud1, pch=16, cex=1, xlab="PercentMarried", ylab="R-student residual", main = "Residual-by-regressor plot")
abline(h=0, lty=1, lwd=3)
# The only other concerning is the fanning in of the 
# medIncome, implying that variance is nonconstant.
par(mfrow = c(2, 2))
plot(train_1$PctBachDeg25_Over, resid_df$rstud1, pch=16, cex=1, xlab="PctBachDeg25_Over", ylab="R-student residual", main = "Residual-by-regressor plot")
abline(h=0, lty=1, lwd=3)
plot(train_1$medIncome, resid_df$rstud1, pch=16, cex=1, xlab="medIncome", ylab="R-student residual", main = "Residual-by-regressor plot")
abline(h=0, lty=1, lwd=3)
plot(train_1$BirthRate, resid_df$rstud1, pch=16, cex=1, xlab="BirthRate", ylab="R-student residual", main = "Residual-by-regressor plot")
abline(h=0, lty=1, lwd=3)
plot(train_1$PctMarriedHouseholds, resid_df$rstud1, pch=16, cex=1, xlab="PctMarriedHouseholds", ylab="R-student residual", main = "Residual-by-regressor plot")
abline(h=0, lty=1, lwd=3)
# The only other concerning is the fanning in of the 
...
PRESS.res <- resid_df$stud1 / (1-resid_df$hat1)
...
high.inf.rows
# [1] 0.6572505 0.7266811 0.6579864 0.6692817 

# the PRESS residual associated with observation i = 302 contributes 1.95% 
